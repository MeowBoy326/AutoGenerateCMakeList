...
input_dir_abs_path = []
for num in input_dir_name.split(" "):
	input_dir_abs_path.append(os.path.join(cur_abs_path, num))
...
for absFilePath in target_dir_list_result:  # 遍历文件夹
    result = absFilePath.split(project_name + "/")
    print(absFilePath)
    if (len(result) > 1) : cmake_file_list_result.append(result[1])
